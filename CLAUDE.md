...
- Top-level singular folders `student/`, `educator/`, `volunteer/`, `employer/`, `admin/` are the **authenticated role dashboards** (each with its own auth-gated `layout.tsx`). The **public marketing pages** (the home page plus `students/`, `educators/`, `clients/`, `employers/`) live in the `(marketing)/` route group. The group name doesn't appear in URLs, so the plural paths (`/students`, `/clients`, …) are unchanged. New public pages go in `(marketing)/` and also need adding to `middleware.ts`'s public-route list. Note that the `client` role doesn't follow the singular/plural pattern: its dashboard is `volunteer/` (`/volunteer/*`) and its marketing and signup page is `(marketing)/clients/` (`/clients`). `login/`, `signup/` and `demo/` are public entry flows and stay top-level.
