...
const navItems = [
  ...
  { href: "/educator/verify", label: "Verify", icon: ClipboardCheck },
  { href: "/educator/profile", label: "Profile", icon: User },
];
...
export default function EducatorLayout({
  ...
  const roleMismatch = !!user && !!role && role !== "educator";
      ...
      <aside className="hidden md:flex md:w-[240px] md:flex-col md:fixed md:inset-y-0 bg-k-white border-r border-k-gray-200">
        ...
        <nav className="flex-1 px-3 py-4 flex flex-col gap-1">
          {navItems.map(({ href, label, icon: Icon }) => {
            const active =
              pathname === href ||
              (href !== "/educator/dashboard" && pathname.startsWith(href));
            return (
              <Link
                key={href}
                href={href}
