...
export default function ProfilePage() {
  ...
  const handleEditOrSave = async () => {
    ...
    setError(null);
    try {
      const res = await apiPatch<ProfileRes>("/api/profile", {
        full_name: name.trim(),
        phone: phone.trim() === "" ? null : phone.trim(),
      });
      setName(res.profile.full_name ?? "");
      setPhone(res.profile.phone ?? "");
      setInstitution(res.profile.institutions?.name ?? institution);
      setEditing(false);
    } catch (err: unknown) {
  ...
  const initials = (name || user?.email || "ST")
    .split(" ")
    .map((n) => n[0])
    .join("")
    .slice(0, 2)
      ...
      <div className="mx-auto max-w-2xl">
        ...
        <div className="mb-6 flex items-center gap-5 rounded-3xl border border-k-gray-200 bg-k-white p-6">
          ...
          <div className="flex-1">
            <h2 className="font-serif text-xl text-k-black">{name || "Student"}</h2>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-k-black">Assigned by institution</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-k-black">Current cohort</p>
              </div>
            </div>
          </div>
        </div>
