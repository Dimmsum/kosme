...
router.get(
  ...
  async (req: AuthRequest, res: Response) => {
    const { data, error } = await supabaseAdmin
      ...
      .select(
        `
      id, name, category_id, service_type_id, notes, reflection_notes, status, created_at, updated_at,
...
router.get("/:id", async (req: AuthRequest, res: Response) => {
  const { data, error } = await supabaseAdmin
    ...
    .select(
      `
      id, name, category_id, service_type_id, notes, reflection_notes, status, created_at, updated_at, is_demo,
      started_at, ended_at, actual_duration_min, duration_tag,
