...
router.get("/students", requireRole("educator"), async (req: AuthRequest, res: Response) => {
...
router.get("/history", requireRole("educator"), async (req: AuthRequest, res: Response) => {
  const { data, error } = await supabaseAdmin
    .from("verifications")
    .select(`
...
router.post("/:serviceId/verify", requireRole("educator"), async (req: AuthRequest, res: Response) => {
  ...
  if (service.status !== "awaiting_educator") {
    return res.status(400).json({ error: "Service is not awaiting educator verification" });
  }
  ...
  const { data: verification, error: verErr } = await supabaseAdmin
    .from("verifications")
    .upsert(
      { service_id: serviceId, educator_id: req.userId!, status: "verified", notes: notes ?? null },
...
router.post("/:serviceId/reject", requireRole("educator"), async (req: AuthRequest, res: Response) => {
  const { serviceId } = req.params;
  const { notes } = req.body;
  ...
  const { data: verification, error: verErr } = await supabaseAdmin
    .from("verifications")
    .upsert(
      { service_id: serviceId, educator_id: req.userId!, status: "rejected", notes: notes ?? null },
      { onConflict: "service_id" }
    )
    .select("id, status, notes, created_at")
    .single();
